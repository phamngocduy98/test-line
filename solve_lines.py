...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--temperature-start", type=float, default=5.0,
        help="Initial SA temperature. Default: 5.0",
    )
    parser.add_argument(
        "--cooling-rate", type=float, default=0.9995,
        help="SA cooling rate per iteration. Default: 0.9995",
    )
    parser.add_argument(
        "--restart-interval", type=int, default=20000,
        help="Restart from best solution every N iterations. Default: 20000",
    )
    parser.add_argument(
...
class Line:
    ...
    # Maintained lazily: recomputed whenever _dirty is True
    ...
    def _recompute(
        ...
        spec = merge_cases(requirement_columns, assigned)
        # merge_cases returns None only on single-select conflict;
        # we prevent that during assignment so this should never be None.
        assert spec is not None, "Line has irreconcilable single-select conflict"
        self._spec = spec
        self._cost = spec_cost(spec, requirement_columns, weights)
        self._dirty = False
        # Single-select key: concrete values for each single-select column
        ss: list[tuple[str, tuple[str, ...]]] = []
        for col in SINGLE_SELECT_COLUMNS:
            if col in spec:
                concrete = tuple(t for t in spec[col] if not is_any(t))
    ...
    def get_spec(
        ...
        if self._dirty:
            self._recompute(cases, requirement_columns, weights)
        return self._spec
    ...
    def cost_if_add(
        ...
        """Return hypothetical cost after adding *case*, or None if incompatible."""
        current_spec = self.get_spec(cases, requirement_columns, weights)
        new_spec = _merge_spec_with_case(current_spec, case, requirement_columns)
        if new_spec is None:
            return None
    ...
    def cost_if_remove(
        ...
        """Return hypothetical cost after removing *case*."""
        remaining = [cases[i] for i in self.case_indices if i != case.index]
        if not remaining:
            return 0.0
        spec = merge_cases(requirement_columns, remaining)
    ...
    def cost_if_swap(
        ...
        """Return hypothetical cost after replacing *remove_case* with *add_case*, or None if incompatible."""
...
class Assignment:
    ...
    def total_cost(
        ...
        return sum(
            line.get_cost(cases, requirement_columns, weights)
...
def _case_base_cost(
    ...
    """Approximate cost of a single test case as a standalone line."""
    spec = {col: case.tokens.get(col, ()) for col in requirement_columns}
    return spec_cost(spec, requirement_columns, weights)
...
def greedy_initial(
    ...
    sorted_cases = sorted(
        cases,
        key=lambda c: (_case_base_cost(c, requirement_columns, weights), c.index),
        reverse=True,
    )

    for case in sorted_cases:
        best_line: Line | None = None
        best_cost_after: float = math.inf
        ...
        for line in active:
            ...
            ss_key = line.get_ss_key(cases, requirement_columns, weights)
            if not _ss_compatible(ss_key, case):
                continue
            cost_after = line.cost_if_add(case, requirement_columns, weights, cases)
            if cost_after is None:
                continue  # merge conflict (shouldn't happen after ss check, but guard)
...
def _try_transfer(
    ...
    if len(from_line.case_indices) <= 1:
        ...
        pass
    ss_key = to_line.get_ss_key(cases, requirement_columns, weights)
    if not _ss_compatible(ss_key, case):
        return None

    cost_from_before = from_line.get_cost(cases, requirement_columns, weights)
    cost_to_before = to_line.get_cost(cases, requirement_columns, weights)

    cost_from_after = from_line.cost_if_remove(case, requirement_columns, weights, cases)
    cost_to_after = to_line.cost_if_add(case, requirement_columns, weights, cases)
    if cost_to_after is None:
        return None
...
def _try_swap(
    ...
    cost_a_before = line_a.get_cost(cases, requirement_columns, weights)
    cost_b_before = line_b.get_cost(cases, requirement_columns, weights)

    cost_a_after = line_a.cost_if_swap(case_b, case_a, requirement_columns, weights, cases)
    cost_b_after = line_b.cost_if_swap(case_a, case_b, requirement_columns, weights, cases)
...
def _try_merge_lines(
    ...
    merged_spec = merge_cases(requirement_columns, combined_cases)
    if merged_spec is None:
        return None

    cost_combined = spec_cost(merged_spec, requirement_columns, weights)
    cost_before = (
        line_a.get_cost(cases, requirement_columns, weights)
        + line_b.get_cost(cases, requirement_columns, weights)
    )
...
def _try_split_line(
    ...
    cost_before = line.get_cost(cases, requirement_columns, weights)

    def eval_split(
        group_a: list[TestCase], group_b: list[TestCase]
    ) -> float | None:
        spec_a = merge_cases(requirement_columns, group_a)
        spec_b = merge_cases(requirement_columns, group_b)
    ...
    sorted_by_cost = sorted(
        line_cases,
        key=lambda c: _case_base_cost(c, requirement_columns, weights),
    )
...
def local_search(
    ...
    best_cost = best.total_cost(cases, requirement_columns, weights)
        ...
        if move_type == "transfer" and len(active) >= 1:
            ...
            for to_line in candidates:
                ...
                d = _try_transfer(
                    ...
                    requirement_columns, weights, max_cases_per_line,
                )
            ...
            delta = _try_swap(
                ...
                cases, requirement_columns, weights,
            )
            ...
            for line_b in candidates:
                d = _try_merge_lines(
                    line_a, line_b, cases, requirement_columns, weights, max_cases_per_line,
                )
                if d is not None and d < best_delta:
                    best_delta = d
                    best_partner = line_b
            ...
            by_cost = sorted(
                active,
                key=lambda l: l.get_cost(cases, requirement_columns, weights),
                reverse=True,
            )
            for line in by_cost[:3]:
                result = _try_split_line(line, cases, requirement_columns, weights, rng)
...
def validate_assignment(
    ...
    for line in assignment.active_lines():
        ...
        spec = line._spec  # already computed
        # 3. RU-band compatibility
        ...
        # 4. Every case is covered by its line's spec
        for idx in line.case_indices:
            case = cases[idx]
            ok, _ = coverage_delta(
...
def write_output(
    ...
    sorted_lines = sorted(
        ...
        key=lambda l: (
            l.get_cost(cases, requirement_columns, weights),
            -len(l.case_indices),
            min(l.case_indices),
        ),
    )
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for line_number, line in enumerate(sorted_lines, start=1):
            spec = line.get_spec(cases, requirement_columns, weights)
            ...
            ru = len(spec.get(RU_COLUMN, ())) if RU_COLUMN in requirement_columns else 0
            ue = numeric_equipment(spec.get(UE_COLUMN, ())) if UE_COLUMN in requirement_columns else 0
            cost = line.get_cost(cases, requirement_columns, weights)
            eq_count = du + ru + ue
...
def main() -> int:
    ...
    initial_cost = initial.total_cost(cases, requirement_columns, weights)
    ...
    final_cost = best.total_cost(cases, requirement_columns, weights)
    ...
    validate_assignment(best, cases, requirement_columns, support, args.max_cases_per_line)

    # Gather unique specs
    from solve_test_lines import spec_signature
    unique_specs: set[tuple] = set()
    for line in final_lines:
        unique_specs.add(spec_signature(line._spec))
